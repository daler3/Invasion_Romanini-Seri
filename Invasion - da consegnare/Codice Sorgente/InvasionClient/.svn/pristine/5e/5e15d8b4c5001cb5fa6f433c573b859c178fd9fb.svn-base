...
public class PanelPartita extends JPanel implements ViewPartita{
	...
	public PanelPartita(ControllerPartita partita) {
		...
		JButton btnAbbandona = new JButton("Abbandona Partita");
		btnAbbandona.addActionListener(new ActionListener() {
			@Override
			public void actionPerformed(ActionEvent e) {
				ControllerLimbo controllerLimbo = controller.abbandonaPartita();
		...
		btnAbbandona.setBounds(12, 9, 168, 18);
		panel_3.add(btnAbbandona);
		
		JButton button_8 = new JButton("Classifica");
		button_8.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent arg0) {
				new FrameClassifica(controller.visualizzaClassifica());
			}
		});
	...
	public void riceviClassificaFinale(ArrayList<String> classificaFinale,
		...
		String testo = "Complimenti sei tra i primi! Questa è la classifica finale: ";
		Classifica classificaCreata = new Classifica(classificaFinale.size());
		int i = 0;
		int punti = 0;
		String nome = "";
		Iterator<String> itClassifica = classificaFinale.iterator();
		while(itClassifica.hasNext()){
			nome = itClassifica.next();
			testo = testo+(i+1)+"°: "+nome+" - Vince: ";
			if(i == 0){
				punti = punteggioPrimo.intValue();
			}
			else if(i == 1){
				punti = punteggioSecondo.intValue();
			}
			else{
				punti = 0;
			}
			i++;
			testo = testo+punti+" punti";
			classificaCreata.aggiungiGiocatore(nome, punti);
			if(itClassifica.hasNext()){
				testo = testo+" -- ";
			}
		}
		lblCronaca.setText(testo);
		new FrameClassifica(classificaCreata);
